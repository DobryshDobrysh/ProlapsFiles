...
function mwLib.overlay(pnl, clOrPnl, persist, col)
...
function mwLib.button(pnl, txt, click)
...
function mwLib.slider(pnl, txt, min, max, prc)
...
function mwLib.checkBox(pnl, txt)
...
function mwLib.textEntry(pnl, txt)

	local t = txt and mwLib.label(pnl, txt) or nil
	if t then
...
function mwLib.numberWang(pnl, txt, val, min, max)
...
function mwLib.colorPicker(pnl, txt, enableAlpha, enablePalette)
...
function mwLib.comboBox(pnl, txt, choices)
...
function mwLib.textEntryIcon(pnl, txt, path)
	...
	function b:DoClick()
		...
		self.picker = mwLib.icons.picker(function(val) e:SetValue(val) end, e:GetValue(), path)
	end
...
function mwLib.confirmDialog(pnl, question, callback, noCancel)
	local fr = mwLib.overlay(pnl, 'DPanel')
	fr:SetSize(180, 60)
	mwLib.label(fr, question):SetContentAlignment(5)
	local btmPan = fr:Add('DPanel')
	btmPan:Dock(FILL)
	btmPan:SetPaintBackground(false)
	local btn = mwLib.button(btmPan, 'Да', function() callback(true) fr:Remove() end)
	btn:Dock(LEFT)
	btn:SetWide(50)
	btn:DockMargin(5, 0, 5, 5)
	local btn = mwLib.button(btmPan, 'Нет', function() callback(false) fr:Remove() end)
	btn:Dock(LEFT)
	btn:SetWide(50)
	btn:DockMargin(5, 0, 5, 5)
	if not noCancel then
		local btn = mwLib.button(btmPan, 'Отмена', function() fr:Remove() end)
		btn:Dock(FILL)
		btn:DockMargin(5, 0, 5, 5)
	end
end

function mwLib.binder(pnl, txt, val)
	local t = txt and mwLib.label(pnl, txt) or nil
	if t then
		t:DockMargin(3, 5, 0, 0)
	end
...
hook.Add('Think', 'mwLib.panels', function()
	hook.Remove('Think', 'mwLib.panels')

	local Panel = FindMetaTable 'Panel'
	surface.CreateFont('mwLib.hint', {
		font = 'Calibri',
		extended = true,
		size = 18,
		weight = 350,
	...
	surface.CreateFont('mwLib.hint-sh', {
		font = 'Calibri',
		extended = true,
		size = 40,
		weight = 350,
	...
	local function paintHint(self, w, h)
		...
		local al = self.anim
		surface.SetFont('mwLib.hint')
		local tw, th = surface.GetTextSize(self.hint)
		local x, y = w / 2, -16*#self.lines

		self.shText = self.shText or ('|'):rep(math.floor((tw+30)/15))
		draw.SimpleText(self.shText, 'mwLib.hint-sh', x, y, color_black, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

		draw.RoundedBox(4, (w-tw-14) / 2, y - 10, tw + 14, -y+6, Color(170,119,102, al*255))
		for i, line in ipairs(self.lines) do
			draw.SimpleText(line, 'mwLib.hint', x, y+(i-1)*16, Color(255,255,255, al*255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		end
