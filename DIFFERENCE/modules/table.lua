mwLib.table = mwLib.table or {}

function mwLib.table.toKeyVal(tbl)
	local out = {}
...
function mwLib.table.count(tbl, func)
	local count = 0
	for k, v in pairs(tbl) do
...
function mwLib.table.map(tbl, func)
	local out = {}
	for k, v in pairs(tbl) do
		out[k] = func(v, k, tbl)
	end
...
function mwLib.table.mapSequential(tbl, func)
	local out = {}
	local index = 1
...
function mwLib.table.strip(tbl, defaults, depth)
	depth = depth or 1
	...
	for k, default in pairs(defaults) do
		...
		elseif istable(tbl[k]) then
			if istable(default) and table.Count(mwLib.table.diff(tbl[k], default)) == 0 then
				mwLib.table.strip(tbl[k], default, depth - 1)
			else
				tbl[k] = nil
			end
		end
...
function mwLib.table.reduce(tbl, func, initVal)
...
function mwLib.table.find(tbl, func)
	for k, v in pairs(tbl) do
		if func(v, k) then return v, k end
	end
end

function mwLib.table.filter(tbl, func)
	local found = {}
	for k, v in pairs(tbl) do
...
function mwLib.table.filterMultiple(tbl, funcs, limit)
	local out = {}
	local foundAmount = 0
	for k, v in pairs(tbl) do
...
function mwLib.table.precacheFilterFunctions(query)
	local funcs = {}
...
function mwLib.table.filterQuery(tbl, query, limit)
	return mwLib.table.filterMultiple(tbl, mwLib.table.precacheFilterFunctions(query), limit)
end

function mwLib.table.diff(t1, t2, changes)
	changes = changes or {}
	...
	for k, v in pairs(t1) do
		if istable(v) then
			if t2[k] ~= nil then
				local innerChanges = {}
				mwLib.table.diff(v, t2[k], innerChanges)
