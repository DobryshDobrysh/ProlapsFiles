...
--[[
	...
	Example:
		...
		local function printNextNumber()
			current = mwLib.math.loop(current + 1, 1, 4)
			print(current)
		end
...
function mwLib.math.loop(val, min, max)
...
--[[
	...
	Example:
		--- Lua
		print(mwLib.math.loopedDiff(10, 25, 0, 360))
		print(mwLib.math.loopedDiff(10, 365, 0, 360)) -- 365 deg is 1 full circle + 5 deg
		---
		> 15
		> -5
]]
function mwLib.math.loopedDiff(value1, value2, min, max)
...
--[[
	Function: distCircular
		Basically just math.abs()'ed version of <mwLib.math.loopedDiff>
]]
function mwLib.math.loopedDist(value1, value2, min, max)
	return math.abs(mwLib.math.loopedDiff(value1, value2, min, max))
end

--[[
	Function: angDelta
		Convenience alias of <mwLib.math.loopedDiff> with
		0 and 360 as min and max limits respectively
]]
function mwLib.math.angleDiff(angle1, angle2)
	return mwLib.math.loopedDiff(angle1, angle2, 0, 360)
end
...
function mwLib.math.sign(value)
	return value > 0 and 1 or value < 0 and -1 or 0
end

function mwLib.math.lerp(src, tgt, fraction, min, max)
	if src == tgt then return src end
	local diff = tgt - src
	local sign = mwLib.math.sign(diff)
...
function mwLib.math.lerpUnclamped(src, tgt, fraction, min, max)
	if src == tgt then return src end
	local diff = tgt - src
	local sign = mwLib.math.sign(diff)
...
function mwLib.math.lerpAngle(src, tgt, fraction, min, max)
	if src == tgt then return src end
	local ch = mwLib.math.angleDiff(src, tgt)
	local sign = mwLib.math.sign(ch)
...
function mwLib.math.lerpVector(src, tgt, fraction, min, max)
	if src == tgt then return src end
	local diff = tgt - src
...
function mwLib.math.inRange(val, min, max, exclusive)
	if exclusive then
...
--[[
	...
	Example:
		--- Lua
		print(mwLib.math.remap(9.81, 9, 10, 1, 100))
		---
		> 81
]]
function mwLib.math.remap(val, oldMin, oldMax, newMin, newMax)
