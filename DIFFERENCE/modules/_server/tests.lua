mwLib.testSectionMap = {}
mwLib.tests = {}

function mwLib.registerTests(data, sectionName)
	if not CFG.dev then return end -- save some memory in production

	local children = sectionName and mwLib.testSectionMap[sectionName].children or mwLib.tests
	if data.children then
		local section = mwLib.testSectionMap[data.name]
		if not section then
			section = {
				name = data.name,
				children = {},
			...
			mwLib.testSectionMap[section.name] = section
			table.insert(children, section)
		end

		for _, v in ipairs(data.children) do
			mwLib.registerTests(v, section.name)
		end
	else
		local test = {
			name = data.name,
		...
		local _, oldKey = mwLib.table.find(children, function(v)
...
function mwLib.runTests(callback)
	local curLevel = -1
	local queue = {}
	...
	for _, v in ipairs(mwLib.tests) do
		addToQueue(v)
	end
...
local errors = {}
hook.Add('mwLib.error', 'tester', function(msg, ply)
...
TESTER_SOCKET = mwLib.socket(uri, CFG.keys.services)
