...
} mwLib.rulesTime = rulesTime

function mwLib.time.formatDuration(sec, rulesOverride)
	return mwLib.string.formatCountExt(sec, rulesOverride or rulesTime)
end
...
function mwLib.time.formatIn(sec, short)
	if short then
		return mwLib.string.formatCountExt(sec, rulesTimeIn)
	else
		return 'через ' .. mwLib.string.formatCountExt(sec, rulesTimeIn)
	end
end
