mwLib.players = mwLib.players or {}

function mwLib.players.resolve(value)
...
function Player:GetEyeTraceLimited(length)
	...
	hook.Run('mwLib.eyeTraceFilter', self, t.filter)

	return util.TraceLine(t)
end

for _, name in ipairs({ 'AccountID', 'SteamID', 'SteamID64', 'UniqueID' }) do
	Player[name] = mwLib.func.detour(Player[name], 'Player:' .. name, function(original, player)
