...
function mwLib.string.stripNonWord(str, ignore)
	local res = ''
...
function mwLib.string.urlEncode(url)
...
function mwLib.string.signed(number)
...
function mwLib.string.separateDigits(number, separator)
...
function mwLib.string.formatCount(num, single, plural, plural2)
	num = math.abs(num)
	if plural2 then
...
function mwLib.string.formatCountExt(num, rules)
	for i, rule in ipairs(rules) do
		local nextRule = rules[i+1]
		if not nextRule or num < nextRule[1] then
			local _num = math.floor(num / rule[1])
			return ('%s %s'):format(_num, mwLib.string.formatCount(_num, unpack(rule, 2)))
		end
	end
end

function mwLib.string.isSteamID(str)
	if not isstring(str) then return false end
	return str:find('^STEAM_[0-5]:[01]:%d+$') ~= nil
end

function mwLib.string.isSteamID64(str)
	if not isstring(str) then return false end
	return str:gmatch('^7656119%d%d%d%d%d%d%d%d%d%d$') ~= nil
end

function mwLib.string.compressSteamID(str)
	if not mwLib.string.isSteamID(str) then return str end
	str = str:Replace(':', '')
	str = str:sub(8)
	return str:sub(1, 1) .. pon.encode({tonumber(str:sub(2))}):sub(3, -3)
end

function mwLib.string.decompressSteamID(str)
	if not isstring(str) or str:len() < 2 then return str end
	if mwLib.string.isSteamID(str) then return str end
...
function mwLib.string.isUrl(str)
	return isstring(str) and str:gsub(urlPattern, '', 1) == ''
end

function mwLib.string.splitByUrl(str)
...
function mwLib.string.random(size, alphabet)
