...
mwLib.func = mwLib.func or {}

function mwLib.func.zero() end
function mwLib.func.yes() return true end
function mwLib.func.no() return false end
...
function mwLib.func.performance(funcs, times)
...
--[[
	...
	Examples:
		--- Lua
		local save = mwLib.func.debounce(function() print('saved') end, 0.5)
...
function mwLib.func.debounce(func, delay)

	local name = tostring(func) .. mwLib.string.uuid()
	return function(...)
		local args = {...}
...
function mwLib.func.debounceStart(func, delay)
...
function mwLib.func.debounceEnd(func, delay)
...
--[[
	...
	Example:
		...
		mwLib.func.throttle(player.GetAll(), 10, 0.2, function(ply)
			print(ply:Name())
		end):Then(function()
...
function mwLib.func.throttle(tbl, steps, minStep, func)

	local stack = mwLib.array.toStack(tbl)
...
--[[
	...
	Example:
		...
		mwLib.func.loop(function(again, lastMsg)
...
function mwLib.func.chain(funcs)

	local stack = mwLib.array.toStack(funcs, true)
...
function mwLib.func.parallel(funcs, amountToWait)
	return util.Promise(function(res, rej)
...
mwLib.func.originals = mwLib.func.originals or {}

function mwLib.func.detour(original, id, func)
	mwLib.func.originals[id] = mwLib.func.originals[id] or original
